...
CREATE TABLE produto(
	...
	forn_primario numeric(9) REFERENCES fornecedor(nif),
	data date NOT NULL,
	categoria varchar(64) NOT NULL REFERENCES categoria(nome),
...
CREATE FUNCTION check_categoria_type_sup() RETURNS trigger AS $check_categoria_type_sup$
	BEGIN
		IF EXISTS (SELECT nome FROM categoria_simples WHERE (NEW.nome = categoria_simples.nome)) THEN
...
CREATE FUNCTION check_categoria_type_simp() RETURNS trigger AS $check_categoria_type_simp$
	BEGIN
		IF EXISTS (SELECT nome FROM super_categoria WHERE (super_categoria.nome = NEW.nome)) THEN
			RAISE EXCEPTION 'Uma categoria nao pode ser simples e super ao mesmo tempo!';
		END IF;
		RETURN NEW;
	END;
$check_categoria_type_simp$ LANGUAGE plpgsql;
CREATE TRIGGER check_categoria_type_simp BEFORE INSERT OR UPDATE ON categoria_simples FOR EACH ROW EXECUTE PROCEDURE check_categoria_type_simp();

CREATE TABLE constituida(
	super_categoria varchar(64) REFERENCES super_categoria(nome) ON DELETE CASCADE,
...
CREATE FUNCTION categoria_verify() RETURNS trigger AS $categoria_verify$
	BEGIN
		IF NEW.super_categoria = NEW.categoria THEN
			RAISE EXCEPTION 'Uma super_categoria nao pode ser constituida por si propria';
		END IF;
		IF COUNT(*) FROM categoria_simples, super_categoria WHERE (NEW.super_categoria = categoria_simples.nome) IS NOT NULL THEN
			RAISE EXCEPTION 'A super_categoria que esta a tentar adicionar e uma categoria_simples!';
		END IF;
...
CREATE TABLE planograma(
	...
	altura varchar(10) NOT NULL CHECK (altura !=''),
...
CREATE FUNCTION fornece_sec_verify() RETURNS trigger AS $fornece_sec_verify$
	BEGIN
		...
		END IF;
		IF COUNT(forn_primario) FROM produto WHERE (NEW.nif = produto.forn_primario) AND (NEW.ean = produto.ean) IS NOT NULL THEN
			RAISE EXCEPTION 'Um fornecedor nao pode ser primario e secundario para o mesmo produto';
		END IF;
		NEW.nif := current_nif;
		NEW.ean := current_ean; /*Qual a funcao disto? */
		RETURN NEW;
	END;
$fornece_sec_verify$ LANGUAGE plpgsql;
CREATE TRIGGER fornece_sec_verify BEFORE INSERT OR UPDATE ON fornece_sec
    FOR EACH ROW EXECUTE PROCEDURE fornece_sec_verify();
